...
2. Before staging, look at what is untracked: a generated directory about to enter git (`node_modules/`, `.wrangler/`, `__pycache__/`, `dist/`, `build/`, `.venv/`, `coverage/`) gets one line appended to `.gitignore` first (`echo "node_modules/" >> .gitignore`). Then stage all changes (`git add -A`) — but if anything looks like a secret (`.env`, keys, credentials), stop and warn the user before committing anything, exactly as `/update-docs-and-commit` would.
