...
allowed-tools: Bash(git add:*), Bash(git commit:*), Bash(git status:*), Bash(git log:*)
---
...
- Changes since last save: !`git status --short | head -20`
...
2. Stage all changes (`git add -A`) — but if anything looks like a secret (`.env`, keys, credentials), leave it out and warn the user.
