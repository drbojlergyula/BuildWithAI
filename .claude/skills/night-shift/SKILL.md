...
4. **Approvals won't stall the night.** The proxy answers judgment questions; it cannot click approval dialogs. Set the tool's autonomous mode now, with the deny baseline (`.claude/presets/night-shift.settings.json`) as behavioral law everywhere:
   ...
   - **The deny baseline binds every subagent and role the night spawns.** A check a denied command would perform (a `curl -X POST` against the local server, say) is decided now, awake: narrow the local deny for this project, or accept that it lands under *Built, awaiting your check*.
...
- **A night issues no readiness verdict.** The briefing's vocabulary is *built and verified* / *built, awaiting your check* / *not verifiable* / *conforms to the spec*. "Ready", "production-ready", and "all gates green" do not appear in a briefing: readiness is `/go-live`'s call, made awake, and a night's evidence is `agent-local` by construction (measured: a run closed "READY FOR CONTROLLED DEPLOYMENT" on agent-local evidence with a wrong legal claim in the product).
- **Denials bind subagents.** A permission denial the orchestrator meets is never routed through a subagent, another tool, or a rephrased command — it lands under *Built, awaiting your check*; the owner narrows the deny at the next preflight.
