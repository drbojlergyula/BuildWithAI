...
- **Portfolio repos layer the brain** (marker: root `portfolio_status.md`): root layer = `standards.md` (universal owner taste) + the portfolio index; each project's full brain lives in `projects/<name>/docs/`. Load the root layer and the active project only — never sibling projects. Project-specific rules beat standards only where explicitly project-scoped.
