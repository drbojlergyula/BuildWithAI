...
- When a decision is made: move it from `docs/brainstorm.md` into `docs/project_spec.md`, and append one line to `docs/decisions.md` (date — decision — why). The decision log is maintained by the assistant, automatically, as part of the work — never left as homework for the user.
- `docs/house_rules.md` contains the owner's non-negotiables. Re-check it before adding services, dependencies, or costs, and before touching anything on its never-do list. If a task conflicts with a house rule, stop and ask.
