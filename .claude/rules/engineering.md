...
- **LOAD-BEARING** — sign-in / session / crypto, permissions, data model or migration, file handling, money, new dependency, external integration, CI or config, and an **external-rule claim** — a user-facing statement or computation of an outside authority's rule, rate, cap, or formula → gates + adversarial verification + a decision card.
...
*By content* (`git diff -U0`, case-insensitive): `session`, `authenticat`, `authoriz`, `permission`, `role`, `token`, `password`, `hash`, `crypt`, `owner_id`, `user_id` in added or changed lines; in user-facing text, an added line with a currency amount, a percentage, or a legal citation (`Art.`, `§`) — the floor for external-rule claims, which prose claims escape. **Path alone is not enough** — measured in a sandbox: an invoice portal's entire session and ownership logic lived in `app/main.py`, whose path trips no trigger; only an unrelated dependency change happened to raise the tier. Real projects put auth in `main.py`, `routes.py`, `api.py`.
...
**Conformance is not correctness.** A test whose expected values come from the spec's own formula proves the code does what the spec says, not that the spec is right — so an external-rule claim's expected values and user-facing wording trace to a **quoted source sentence** (URL, read date) in `docs/reference/`, inferences beyond the quote are labelled as inferences, and the verifier's `Oracle:` line says which kind of check ran (`spec-only` is reported as "conforms to the spec", never "correct"). Measured why: "only 25 of 40 points count toward the cap" became "the other 15 are uncapped" in the architecture, the copy, and the tests; an isolated verifier re-derived the same spec and passed it; the authority's FAQ caps those points separately.

**Dangerous error classes get zero, not a rate** — an error the design itself calls unsafe is gated at zero with a named allowlist, not a percentage; and a confidence flag degrades the output rather than annotating it. Measured why: a cross-check passed at "0.39 % merges" while its docstring called a merge dangerous, and low-confidence matches still became the highlighted recommendation.
