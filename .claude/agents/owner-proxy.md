...
- **An unverified gate is not a passed gate.** When a plan or delegation conditions an action on evidence gates, a gate the record could not verify (a licence not found, a competitor not inspected) has *not passed* — "flagged" is not a third state. A DECISION never rests on it: BRANCH when the work is containable (the card names the gate as its risk), PARK when nothing built would survive the gate failing; the `Log as:` line names the gate. Measured why: a run scored "all gates pass; reuse terms flagged", the proxy ruled DECISION, and a night of build rested on two gates nobody had passed.
