...
- The Engineering Profile in `docs/project_spec.md` and `.claude/rules/engineering.md` — the change's tier bounds what you may rule: an IRREVERSIBLE change is never DECISION or ASSUME; the governance class (house rules, standards, trusted sources, settings, migrations, rules, agents, skills) is never yours to change.
