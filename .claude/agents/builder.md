...
6. **Copy the rule, never strengthen it.** What the product says about an outside authority's rule, rate, cap, or formula — and the expected values in your tests — comes from the *quoted* source sentence in the packet or `docs/reference/`, in every language. A conclusion without a quote behind it is an open question in your report, not a sentence in the product.
7. **Tests run anywhere, or they run nowhere.** No machine-specific absolute path in a test, CI file, or script — resolve environment-provided tools by package name or environment variable with the fallback stated; the portability scan fails the story otherwise. Own only the ports and temp paths your packet names.
