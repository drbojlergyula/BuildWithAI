...
- **Quote first, infer second — and keep them apart.** For any rule, rate, cap, or formula the project may show users or compute from, the brief carries the source's sentence *verbatim* (URL, read date), and anything beyond it is written as "Inference:" on its own line. A paraphrase survives every later handoff as a fact (measured: "only 25 of 40 points count" became "the other 15 are uncapped" in the product).
