...
- **A denial is a boundary, not an obstacle.** A check blocked by a permission rule the session runs under goes under *Not verifiable* with the exact command for the owner — never around the denial via another tool, a rephrased command, or a subagent that "isn't bound".
