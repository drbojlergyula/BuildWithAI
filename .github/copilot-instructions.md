...
- **Cost tiering:** the `model:` frontmatter in agent files is Claude Code-specific and has no effect in Copilot. The intent still applies: `spec-reviewer`, `build-verifier`, and `research-analyst` are routine-work roles suited to a mid-tier model in Copilot's model picker; save premium-model requests for `project-advisor` work.
