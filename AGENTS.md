...
The spec and architecture are the source of truth. After meaningful work: update the changelog and status. After a decision: promote it from brainstorm to spec and append one line to the decision log. `docs/house_rules.md` is binding **once it holds real rules** — while it still carries the active `<!-- house-rules: unset -->` comment (a prose mention of the marker does not count) or the example content, there are no house rules yet and the owner's current instructions govern; if a task conflicts with a real house rule, stop and ask. Complex projects split oversized docs hub-and-spoke: the main files stay short indexes, depth lives in `docs/spec/` and `docs/architecture/` pages opened only when the work touches them. Documentation conventions: `.claude/rules/documentation.md`.

**Engineering by risk** (`.claude/rules/engineering.md`): every project carries a plain-English Engineering Profile in its spec, inferred — never asked as a questionnaire. Each change is tiered — ROUTINE (build and verify, nothing added), LOAD-BEARING (evidence gates: tests, secret scan, lockfile, dependency audit, migration rehearsal), IRREVERSIBLE (owner approval) — with deterministic path triggers as the floor. Agents make claims; gates produce evidence; every evidence line says who ran it — and says what it was checked against: a test whose expected values come from the spec's own formula proves conformance, not correctness, so anything the product states about an outside authority's rule traces to a quoted source, never a paraphrase. Business questions are asked only when risky *and* not inferable *and* decisive, at feature entry, once. Go-live judges *fitness for intended use* and says plainly when a product is above what the template can honestly certify.
...
**Spec first — except when the spec is an output.** The normal order is spec → build. For products whose viability is an empirical question (does the data exist? does it fit the platform limit?), it is legitimate to build and measure first, then write the spec from what the build proved — say so in the decision log rather than back-dating a spec you could not have written honestly.

**Owner-driven vs. autonomous.** `save-point`, `update-docs-and-commit`, `go-back`, `night-shift` and `add-project` exist for an owner steering the session turn by turn. During an autonomous run, follow their *outcomes* — commit often, keep the docs and decision log current — without stopping to invoke them; loading a skill that tells you to do what you are already doing costs context and gains nothing.
