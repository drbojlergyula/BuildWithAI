...
- **Skills are native.** The workflows in `.claude/skills/` are real slash commands here (`/start`, `/adopt-project`, `/build-next`, `/new-feature`, `/save-point`, `/go-back`, `/update-docs-and-commit`, `/put-me-in-context`, `/doc-sync-check`, `/fix-bug`, `/go-live`, `/night-shift`, `/template-update`, `/add-project`). Invoke them directly; they may also trigger automatically when relevant — except `/go-back`, `/night-shift`, and `/add-project`, which only the user may invoke.
...
- **The AI team runs as real subagents.** `project-advisor`, `spec-reviewer`, `build-verifier`, `research-analyst`, `owner-proxy`, and `builder` in `.claude/agents/` execute in their own context with scoped tools. The advisor keeps persistent memory in `.claude/agent-memory/` — expect it to remember past reviews. The cost tiering described in `AGENTS.md` is enforced automatically here via each agent's `model` frontmatter.
