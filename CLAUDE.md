...
- **The AI team runs as real subagents.** `project-advisor`, `spec-reviewer`, `build-verifier`, `research-analyst`, and `owner-proxy` in `.claude/agents/` execute in their own context with scoped tools. The advisor keeps persistent memory in `.claude/agent-memory/` — expect it to remember past reviews. The cost tiering described in `AGENTS.md` is enforced automatically here via each agent's `model` frontmatter.
