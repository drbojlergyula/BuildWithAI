...
- **The team is cost-tiered.** `spec-reviewer`, `build-verifier`, and `research-analyst` are pinned to Sonnet so routine checks don't bill at frontier rates; only `project-advisor` inherits the session model, because judgment is what's worth paying up for.
